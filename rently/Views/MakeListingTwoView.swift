...
struct MakeListingTwoView: View {
    ...
    private func saveListingAndRedirect() {
        ...
        // Update the draft
      draft.maxRentalDuration = maxRentalDuration // Assign directly, no rawValue needed
      draft.pickupLocations = selectedLocations // Assign directly, no map needed


        FirebaseService.shared.saveListingFromDraft(draft, userID: user.id!) { result in
            isSaving = false
            switch result {
            case .success:
                nextScreen = true
